...
class LabelList:  # should've been named segment in hindsight...
    ...
    def to_phone_nums_string(self, lang):  # phoneme separations
        ...
        for i in range(len(self.labels)):
            ...
            if l.phone in lang['vowels']:
                ...
                if prev_l.phone in lang[
                    'liquids']:  # check liquids before vowel. move position if liquid with consonant before is found
    ...
    def to_midi_strings(self, x, fs, split_pos, pitch='parselmouth', time_step=0.005, f0_min=40, f0_max=1100,
                        voicing_threshold=0.45, cents=False):  # midi estimation
        global pauses
        pps = 1 / time_step
        f0 = pitch
        if isinstance(pitch, str):
            f0 = get_pitch(x, fs, pe=pitch, time_step=time_step, f0_min=f0_min, f0_max=f0_max,
                           voicing_threshold=voicing_threshold)  # get pitch
        ...
        temp_label = deepcopy(self) - self.labels[
            0].start  # offset label to have it start at 0 because this receives segmented wavs
    ...
    def detect_breath(self, x, fs, time_step=0.005, f0_min=40, f0_max=1100, voicing_threshold=0.6, window=0.05,
                      min_len=0.1, min_db=-60, min_centroid=2000):
        # Referenced from MakeDiffSinger/acoustic_forced_alignment/enhance_tg.py
        global pauses
        # Features needed
        sound = pm.Sound(x, sampling_frequency=fs)
        f0 = get_pitch(x, fs, time_step=time_step, f0_min=f0_min, f0_max=f0_max,
                       voicing_threshold=voicing_threshold)  # VUVs
        hop_size = int(time_step * fs)
        centroid = librosa.feature.spectral_centroid(y=x, sr=fs, hop_length=hop_size).squeeze(0)  # centroid
        rms = librosa.amplitude_to_db(
            librosa.feature.rms(y=x, hop_length=hop_size).squeeze(0))  # RMS for peak/dip searching
        ...
        for lab in temp_label.labels:
            ...
            for ap_start, ap_end in clean_ap_ranges:
                s = int((
                                    ap_start + window) / time_step)  # add window to remove potential energy spike from voiced section before the pause
                ...
                peaks = signal.find_peaks_cwt(rms[s:e], np.arange(6,
                                                                  10)) + s  # if successful, it finds the breath peak better than argmax
                if peaks.size != 0:
                ...
                if e - s - 1 >= max_silences:
                    ...
                    if temp:
                        ...
                        if len(shorter) > 0:
                            ...
                            logging.warning(
                                'A segment could not be shortened to the given maximum length, this sample might be slightly longer than the maximum length you desire.')
                            k = 1
                            while temp is None:
...
def get_pitch(x, fs, pe='parselmouth', time_step=0.005, f0_min=40, f0_max=1100,
              voicing_threshold=0.45):  # parselmouth F0
...
def write_ds(loc, wav, fs, pitch='parselmouth', time_step=0.005, f0_min=40, f0_max=1100, voicing_threshold=0.45,
             **kwargs):
    res = {'offset': 0}
    ...
    f0 = pitch
    if isinstance(pitch, str):
        f0 = get_pitch(wav, fs, pe=pitch, time_step=time_step, f0_min=f0_min, f0_max=f0_max,
...
def process_lab_wav_pair(segment_loc, lab, wav, args, lang=None):
    ...
    segments = read_label(lab).segment_label(max_length=args.max_length, max_silences=args.max_silences,
                                             length_relax=args.max_length_relaxation_factor)
...
if __name__ == '__main__':
    try:
        ...
        parser.add_argument('--max-length', '-l', type=float, default=15,
                            help='The maximum length of the samples in seconds.')
        parser.add_argument('--max-length-relaxation-factor', '-R', type=float, default=0.1,
                            help='This length in seconds will be continuously added to the maximum length for segments that are too long for the maximum length to cut.')
        parser.add_argument('--max-silences', '-s', type=int, default=0,
                            help='The maximum amount of silences (pau) in the middle of each segment. Set to a big amount to maximize segment lengths.')
        parser.add_argument('--max-sp-length', '-S', type=float, default=0.5,
                            help='The maximum length for silences (pau) to turn into SP. Ignored when breath detection is enabled. Only here for fallback.')
        parser.add_argument('--audio-sample-rate', '-r', type=int, default=44100,
                            help='The sampling rate in Hz to put the audio files in. If the sampling rates do not match it will be converted to the specified sampling rate. Enter 0 to ignore sample rates.')
        parser.add_argument('--language-def', '-L', type=str, metavar='path',
                            help='The path of the language definition .json file. If present, phoneme numbers will be added.')
        parser.add_argument('--estimate-midi', '-m', action='store_true',
                            help='Whether to estimate MIDI or not. Only works if a language definition is added for note splitting.')
        parser.add_argument('--use-cents', '-c', action='store_true', help='Add cent offsets for MIDI estimation.')
        parser.add_argument('--pitch-extractor', '-p', type=str, metavar='parselmouth | harvest', default='parselmouth',
                            help='Pitch extractor used for MIDI estimation. Only parselmouth reads voicing-threshold-midi.')
        parser.add_argument('--time-step', '-t', type=float, default=0.005,
                            help='The time step used for all frame-by-frame analysis functions.')
        parser.add_argument('--f0-min', '-f', type=float, default=40,
                            help='The minimum F0 to detect in Hz. Used in MIDI estimation and breath detection.')
        parser.add_argument('--f0-max', '-F', type=float, default=1100,
                            help='The maximum F0 to detect in Hz. Used in MIDI estimation and breath detection.')
        parser.add_argument('--voicing-threshold-midi', '-V', type=float, default=0.45,
                            help='The voicing threshold used for MIDI estimation.')
        parser.add_argument('--detect-breaths', '-B', action='store_true', help='Detect breaths within all pauses.')
        parser.add_argument('--voicing-threshold-breath', '-v', type=float, default=0.6,
                            help='The voicing threshold used for breath detection.')
        parser.add_argument('--breath-window-size', '-W', type=float, default=0.05,
                            help='The size of the window in seconds for breath detection.')
        parser.add_argument('--breath-min-length', '-b', type=float, default=0.1,
                            help='The minimum length of a breath in seconds.')
        parser.add_argument('--breath-db-threshold', '-e', type=float, default=-60,
                            help='The threshold in the RMS of the signal in dB to detect a breath.')
        parser.add_argument('--breath-centroid-threshold', '-C', type=float, default=2000,
                            help='The threshold in the spectral centroid of the signal in Hz to detect a breath.')
        parser.add_argument('--write-ds', '-D', action='store_true',
                            help='Write .ds files for usage with SlurCutter or for preprocessing.')
        parser.add_argument('--write-labels', '-w', type=str, metavar='htk | aud',
                            help='Write labels if you want to check segmentation labels. "htk" gives HTK style labels, "aud" gives Audacity style labels.')
        parser.add_argument('--num-processes', '-T', type=int, default=1,
                            help='Number of processes to run for faster segmentation. Enter 0 to use all cores.')
        ...
        for i in lab_locs:
            ...
            if len(temp) > 1:
                logging.warning(
                    f'Found more than one instance of a wave file equivalent for {file}. Picking {temp[0]}.')
            lab_wav[i] = temp[0]
        ...
        if args.num_processes == 1:
            ...
            with futures.ProcessPoolExecutor(max_workers=workers) as executor:
                results = executor.map(process_lab_wav_pair, itertools.repeat(segment_loc), lab_wav.keys(),
                                       lab_wav.values(), itertools.repeat(args), itertools.repeat(lang))
            logging.info('Writing all transcripts.')
            for res in results:
